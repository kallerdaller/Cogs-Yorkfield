...
class Russianroulette:
    ...
    async def russianroulette(self, ctx, type):
        ...
        if type.lower() == "start":
            ...
            if (user.id == self.json_data["Players"]["1"]) or (str(user.id) == "170571485310615553"):
    ...
    async def startGame(self, bank, ctx, totalPlayers):
        ...
        print(self.leaderboard["Leaderboard"]["Player"])
...
def check_files(): 
    ...
    leaderboard_json = {"Leaderboard": {"Player": {"1": {"Name": "",
                                                   "Wins": "0",
                                                   "Earnings": "0"}}}}
